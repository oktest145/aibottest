...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Rashmika AI Already Activated In This Chat")
            return
        await lel.edit(
            f"Rashmika AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            await lel.edit("Rashmika AI Was Not Activated In This Chat")
            return
        await lel.edit(
            f"Rashmika AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
@daisyx.on_message(
    filters.regex("rashmika|bot|rashmika ai|veracity|veracitymusic")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
async def inuka(client, message):
    ...
    result = result.replace('{"cnt":"', "")
    result = result.replace('"}', "")
    result = result.replace("Aco", "Queer")
    result = result.replace("<a href=\\", "<a href =")
    result = result.replace("<\/a>", "</a>")
    pro = result
    if not "en" in lan and not lan == "":
